# W2-d7 component formal-target feasibility report (v3) — RESULT: PASS

Supersedes:
- `FEASIBILITY_REPORT.md` (v1: documented obsolete implication theorem and ±0.05 band)
- `FEASIBILITY_REPORT_v2.md` (v2: contained minor reporting defects noted in FABLE-003: spliced error message in §2 and typo in S_0 value in §3)
...
Three synthetic datasets were generated to test all discrimination axes of the v3 conjunction theorem:

### 1. Positive Synthetic Run (`triples_pos.json`, SHA-256: `71f0d986...`)
...
- Wall-clock time:
  * Warm cache: ~0.35s (Peak RSS: ~475 MB)
  * Cold first invocation in fresh environment: ~19.1s (dominated by Lean binary/dependency load)
- Exit code: 0
- `#print axioms` output:
...
### 2. Band-Negative Run (`triples_band_neg.json`, SHA-256: `57f9898e...`)
...
- Exit code: 1
- Lean error output (verbatim):
  ```text
  W2D7_band_neg_v3.lean:72:2: error: Tactic `decide` proved that the proposition
    lenOk = true ∧ allValid = true ∧ denomPositive = true ∧ lowerOk = true ∧ upperOk = true
  is false
...
### 3. Guard-Corruption Run (`triples_corrupt.json`, SHA-256: `a318d7b1...`)
- Triple 0 corrupted to $Q_0=1$ with $n_0=50000$ and $S_0=233640$, such that $n_0 \cdot Q_0 < S_0^2$ ($50000 \cdot 1 < 233640^2 = 54587649600$), violating `validTriple`
- Exit code: 1
- Lean error output (verbatim):
  ```text
  W2D7_corrupt_v3.lean:72:2: error: Tactic `decide` proved that the proposition
    lenOk = true ∧ allValid = true ∧ denomPositive = true ∧ lowerOk = true ∧ upperOk = true
  is false
...
For the 30 triples of the $W2, d=7$ component, the reduction executes well within default limits in ~0.35s warm.
...
Therefore, no claim of universal scalability beyond length 30 is made for this v3 certificate without setting `maxRecDepth`.
...
## PASS Criterion
- Wall clock time: ~0.35s warm (< 60s threshold) — **PASS**
...
=> **GO for W2-d7 v3 Formal Certificate.**
